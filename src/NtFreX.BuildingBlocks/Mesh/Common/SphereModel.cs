...
public static class SphereModel
{
    ...
    public static MeshRenderer Create(
        ...
        float red = 0f, float green = 0f, float blue = 0f, float alpha = 1f, float radius = 1f, int sectorCount = 5, int stackCount = 5, TextureView? texture = null, MaterialInfo? material = null,
        string? name = null, DeviceBufferPool? deviceBufferPool = null)
    {
        ...
        return MeshRenderer.Create(graphicsDevice, resourceFactory, graphicsSystem, mesh, shape: new Sphere(radius), transform: transform, textureView: texture, name: name, deviceBufferPool: deviceBufferPool);
    }
