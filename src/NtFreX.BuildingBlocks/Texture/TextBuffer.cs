...
namespace NtFreX.BuildingBlocks.Texture
{
    public class TextBuffer
    {
        private class TextBufferPart
        {
            ...
            public readonly List<VertexPositionColorNormalTexture> Vertices = new List<VertexPositionColorNormalTexture>();
            ...
            try
            {
                ...
                for (var i = 0; i < text.Length; i++)
                {
                    ...
                    else if (!charactersToIgnore.Contains(text[i]))
                    {
                        ...
                        part.Vertices.Add(new VertexPositionColorNormalTexture(new Vector3(cursorPosition.X, cursorPosition.Y + atlas.LineHeight, 0), color, new Vector2(position.X / atlas.Size.Width, 0)));
                        part.Vertices.Add(new VertexPositionColorNormalTexture(new Vector3(cursorPosition.X, cursorPosition.Y, 0), color, new Vector2(position.X / atlas.Size.Width, position.Height / atlas.Size.Height)));
                        part.Vertices.Add(new VertexPositionColorNormalTexture(new Vector3(cursorPosition.X + position.Width, cursorPosition.Y + atlas.LineHeight, 0), color, new Vector2((position.X + position.Width) / atlas.Size.Width, 0)));
                        part.Vertices.Add(new VertexPositionColorNormalTexture(new Vector3(cursorPosition.X + position.Width, cursorPosition.Y, 0), color, new Vector2((position.X + position.Width) / atlas.Size.Width, position.Height / atlas.Size.Height)));
        ...
        public MeshDeviceBuffer[] Build(GraphicsDevice graphicsDevice, ResourceFactory resourceFactory, out Span<MeshDeviceBuffer> data, DeviceBufferPool? deviceBufferPool = null, CommandListPool? commandListPool = null)
        {
            ...
            foreach(var part in textBufferParts)
            {
                ...
                var mesh = new MeshDataProvider<VertexPositionColorNormalTexture, Index16>(part.Vertices.ToArray(), Array.Empty<Index16>(), PrimitiveTopology.TriangleList);
